...
void decretaPunteggio(int mano[][NM_COL_MANO], bool* reale, bool* poker, bool* colore, bool* scala, bool* tris, int* coppie);
void stampaPunteggio(const bool* reale, const bool* poker, const bool* colore, const bool* scala, const bool* tris, const int* coppie);
...
int main(void){
    ...
    bool reale, poker, colore, scala, tris;
    int  coppie = 0;
    int mano[NM_CARTE_MANO][NM_COL_MANO];
    
    while(true){
        leggiCarte(mano);
        decretaPunteggio(mano, &reale, &poker, &colore, &scala, &tris, &coppie);
        stampaPunteggio(&reale, &poker, &colore, &scala, &tris, &coppie);
    }
...
void leggiCarte(int mano[][NM_COL_MANO]){
    ...
    while(carteLette < NM_CARTE_MANO){
        ...
        for(int i = 0; i < carteLette; i++){
            if(mano[i][IDX_VAL] == iVal && mano[i][IDX_SEME] == iSeme){
                cartaDuplicata = true;
                break;
            }
        }
        ...
        // salvo la carta
        mano[carteLette][IDX_VAL] = iVal;
        mano[carteLette][IDX_SEME] = iSeme;
        carteLette++;
    }
}

void decretaPunteggio(int mano[][NM_COL_MANO], bool* reale, bool* poker, bool* colore, bool* scala, bool* tris, int* coppie){
    *reale = *poker = *scala = *tris = false;
    *coppie = 0;
    ...
    *colore = true;
    for(int carta = 0; carta < NM_CARTE_MANO - 1; carta++){
        if(mano[carta][IDX_SEME] != mano[carta + 1][IDX_SEME]){
            *colore = false;
            break;
        }
    }
    ...
    for(int carta = 0; carta < NM_CARTE_MANO - 1; carta++){
        ...
        if(mano[carta][IDX_VAL] == mano[carta + 1][IDX_VAL] - 1){
            carteConsecutive++;

            // controllo Scala Minima
            if(carteConsecutive == 4 && mano[0][IDX_VAL] == 0 && mano[NM_CARTE_MANO - 1][IDX_VAL] == 12){
                *scala = true;
            }
        ...
        if(mano[carta][IDX_VAL] == mano[carta + 1][IDX_VAL]){
            carteUguali++;
...
void selectionSort(int a[][NM_COL_MANO], int n){
    ...
    for(int i = 0; i < n; i++){
        if(a[idxMax][IDX_VAL] < a[i][IDX_VAL]){
            idxMax = i;
        }
    }

    tempVal = a[n - 1][IDX_VAL];
    tempSeme = a[n - 1][IDX_SEME];

    a[n - 1][IDX_VAL] = a[idxMax][IDX_VAL];
    a[n - 1][IDX_SEME] = a[idxMax][IDX_SEME];
    
    a[idxMax][IDX_VAL] = tempVal;
    a[idxMax][IDX_SEME] = tempSeme;
